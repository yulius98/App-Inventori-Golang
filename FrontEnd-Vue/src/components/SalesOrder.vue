<template>
  <div class="p-6">
    <h1 class="text-2xl font-bold mb-6">{{ 'Sales Order' }}</h1>
...
const simpan = async () => {
  ...
  try {
    ...
    ElMessage.success('Sales Order berhasil diproses')
    emit('success')
  } catch (error) {
    //console.error('Error details:', error)
    //console.error('Response data:', error.response?.data)
    //console.error('Response status:', error.response?.status)
    ElMessage.error(error.response?.data?.message || 'Sales Order Gagal')
  } finally {
    loading.value = false
  }
}
