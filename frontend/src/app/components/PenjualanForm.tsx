...
export default function PenjualanForm({ onSubmit }: Props) {
  ...
  const handleFormSubmit = (data: NewStok) => {
    ...
    const formattedData: NewStok = {
      ...
      qty_in: 0, // Selalu 0 untuk pemnjualan
      qty_out: Number(data.qty_out) 
    };
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 max-w-md mx-auto p-4 border border-gray-300 rounded">
      <div className="flex items-center gap-2 mb-4">
        <h3 className="text-lg font-medium">Penjualan</h3>
      </div>
      ...
      <div>
        <label className="block font-medium mb-1">Jumlah Produk yang Dijual</label>
        <input 
          type="number" 
          {...register('qty_out', { required: true, min: 1 })} 
          placeholder="Jumlah" 
          className="border p-2 w-full"
          suppressHydrationWarning={true}
        />
